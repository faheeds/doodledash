...
export default function MultiResultScreen({ navigation, route }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const sortedTallies = Object.values(map).sort((a, b) => b.total - a.total);
      setTallies(sortedTallies);

      // Award Style Stars for votes received by this player
      const myTally = sortedTallies.find(t => t.display_name === username);
      if (myTally && myTally.total > 0) {
        await Storage.addStyleStars(myTally.total);
      }
